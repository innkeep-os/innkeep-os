...
2. **Groom the keeper's files.** `innkeeper/notes/` (drop notes about parked/retired surfaces; merge duplicates) and `innkeeper/learnings.md` (same — merge duplicates, drop lessons about surfaces that no longer exist). **It has no length limit and nothing is promoted out of it**: unlike `me/learnings.md` it stages nothing, because every line is already a concrete correction. Grooming it means tightening, not splitting. `charter.md` and `state.md` were archived 2026-09-09 — the Kitchen injected the first and the companion app derives the second.
...
6. **Journal the sweep.** Append a short dense entry to today's `journal/YYYY-MM-DD.md`: what was groomed, reconciled, swept, and anything left for your review.
