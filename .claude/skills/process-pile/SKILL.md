...
2. First decide per item: **is the file itself the artifact, or is it a capture to dissolve?**
   ...
   - **Capture to dissolve** (a loose thought/list whose content belongs inside existing files) → route the content:
     - Actionable → the right project's own `loops.md` — one list per place, whether it is a call waiting on you or work waiting on anyone (CLAUDE.md). If it belongs to no project, the root `loops.md` — there is one list per place and this is the workspace's own.
...
4. Nothing to update afterwards: there is no hand-kept dashboard. There's no hand-kept "Now" any more — the companion app derives it, so keeping `loops.md` honest is what makes it true.
