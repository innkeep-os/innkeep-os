...
2. First decide per item: **is the file itself the artifact, or is it a capture to dissolve?**
   ...
   - **Capture to dissolve** (a loose thought/list whose content belongs inside existing files) → route the content:
     - Actionable → the right project's own `loops.md` — one list per place, whether it is a call waiting on you or work waiting on anyone (CLAUDE.md). If it belongs to no project: `tasks/loops.md` for work, the root `loops.md` for something waiting on you.
...
4. Update `HOME.md` (Active projects / Recently) if anything changed the big picture. There's no hand-kept "Now" any more — the companion app derives it, so keeping `loops.md` honest is what makes it true.
